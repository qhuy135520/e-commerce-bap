...
import { DividerTitle, NavlinkStyled, HeadingStyled } from "@/components";

import { useUpdateUser } from "@/hooks/authentication/useUpdateUser";

export default function UpdateUserForm() {
  const { isUpdating, UpdateUserSchema, t, handleSubmit, initialValues } = useUpdateUser();

  return (
    <Spin spinning={isUpdating}>
      <HeadingStyled as="h1">{t("updateUser.title")}</HeadingStyled>
      <DividerTitle title={t("updateUser.title")} />
      <ConfigProvider
        theme={{
          components: {
            Form: {
      ...
      >
        ...
        >
          <Form layout="vertical" autoCapitalize="off">
            ...
            <Form.Item name="birthdate" label={t("updateUser.form.birthdateLabel")}>
              <DatePicker
                name="birthdate"
                format="YYYY-MM-DD"
                disabled={isUpdating}
            ...
            <Form.Item name="password" label={t("updateUser.form.passwordLabel")}>
              <Input.Password
                name="password"
